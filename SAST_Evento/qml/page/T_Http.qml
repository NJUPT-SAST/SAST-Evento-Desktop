        ...
        Column {
            ...
            FluButton {
                ...
                onClicked: {
                    ...
                    var param = "我命由我不由天"
                    http.postString("https://httpbingo.org/post",
                                    callable, param)
                }
            }
            FluButton {
    ...
    FolderDialog {
        ...
        onAccepted: {
            ...
            callable.onDownloadProgress = function (recv, total) {
                ...
                text_file_size.text = "%1/%2".arg(locale.formattedDataSize(
                                                      recv)).arg(
                            locale.formattedDataSize(total))
                layout_file_size.visible = true
            }
            var path = FluTools.toLocalPath(
                        currentFolder) + "/big_buck_bunny.mp4"
            http.download("http://clips.vorwaerts-gmbh.de/big_buck_bunny.mp4",
                          callable, path)
        }
    }

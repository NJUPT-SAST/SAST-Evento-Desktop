...
class repositoryImpl : public Repository {
   public:
    ...
    QStringList get_permitted_event(
        EventoException& err) override;  // Test completed
    DTO_Permission get_event_permission(
        EventoID event, EventoException& err) override;  // Test completed
    DTO_User get_user_info(const UserID& id,
                           EventoException& err) override;  // Test completed
    std::vector<DTO_Slide> get_home_slide_list(
        const int& size, EventoException& err) override;  // Test completed

    // event-fetch
    std::vector<DTO_Evento> get_undertaking_list(
        EventoException& err) override;  // Test completed
    std::vector<DTO_Evento> get_latest_list(
        EventoException& err) override;  // Test completed
    std::vector<DTO_Evento> get_subscribed_list(
        EventoException& err) override;  // Test completed
    std::vector<DTO_Evento> get_history_list(
        EventoException& err) override;  // Test completed
    std::vector<DTO_Evento> get_event_list(
        const int& page, const int& size,
        EventoException& err) override;  // Test completed
    DTO_Evento get_event(EventoID event,
                         EventoException& err) override;  // Test completed
    std::vector<DTO_Feedback> get_feedback_list(
        EventoException& err) override;  // Test completed
    std::vector<DTO_Slide> get_slide_list(
        EventoException& err) override;  // Test completed
    std::vector<DTO_Slide> get_event_slide_list(
        EventoID id, EventoException& err) override;  // Test completed
    std::vector<EventType> get_type_list(const int& page, const int& size,
                                         EventoException& err) override;
    std::vector<Location> get_location_list(
        EventoException& err) override;  // Test completed
    QString get_qrcode(const int& eventId,
                       EventoException& err) override;  // Test completed

    // event-upload
    bool event_checkin(EventoID event, const QString& code,
                       EventoException& err) override;  // Test completed
    bool event_feedback(const DTO_Feedback& code,
                        EventoException& err) override;  // Test completed
    bool event_subscribe(EventoID event,
                         EventoException& err) override;  // Test completed

    // admin-fetch
    std::vector<DTO_Evento> get_qualified_event(
        int type, const std::vector<int>& dep, const QDate& day,
        EventoException& err) override;  // Test completed
   ...
   private:
    ...
    QString readFile(const QString& file_s, const QString& tag,
                     const std::pair<QString, QString>& key) {
        ...
        if (jsonError.error == QJsonParseError::NoError && jsonDoc.isArray()) {
            ...
            for (int i = 0; i < jsonArr.count(); i++) {
                ...
                if ((!key.first.compare("null") &&
                     !key.second.compare("null")) ||
                    (unit.contains(key.first) &&
                     !unit.value(key.first).toString().compare(key.second))) {
    ...
    std::pair<QString, QString> locationHelper(const QJsonArray& locationList,
                                               QString id) {
        for (int i = 0; i < locationList.count(); i++) {
            if (!locationList.at(i).toObject().value("id").toString().compare(
                    id)) {
                return std::pair<QString, QString>(locationList.at(i)
                                                       .toObject()
                                                       .value("location_name")
                                                       .toString(),
                                                   locationList.at(i)
        ...
        if (jsonError.error == QJsonParseError::NoError && jsonDoc.isArray()) {
            ...
            for (int i = 0; i < jsonArr.count(); i++) {
                ...
                if ((!key.first.compare("null") &&
                    ...
                    QString eventoId = unit.contains("id")
                                           ? unit.value("id").toString()
                                           : nullptr;
                    QStringList departmentIds =
                        readFile(
                            "localdata/event_department.json", "department_id",
                            std::pair<QString, QString>("event_id", eventoId))
                            .split(" ");
                    for (int i = 0; i < departmentIds.count(); i++) {
                        departments.push_back(Department{
                            departmentIds[i].toInt(),
                            readFile("localdata/department.json",
                                     "department_name",
                                     std::pair<QString, QString>(
                                         "id", departmentIds.at(i)))});
                    }
                    res.push_back(DTO_Evento{
                        eventoId.toInt(),
                        unit.contains("title") ? unit.value("title").toString()
                                               : nullptr,
                        unit.contains("description")
                            ? unit.value("id").toString()
                            : nullptr,
                        unit.contains("gmt_event_start")
                            ? QDateTime::fromString(
                                  unit.value("gmt_event_start").toString(),
                                  "yyyy-MM-dd hh:mm:ss")
                            : QDateTime(),
                        unit.contains("gmt_event_end")
                            ? QDateTime::fromString(
                                  unit.value("gmt_event_end").toString(),
                                  "yyyy-MM-dd hh:mm:ss")
                            : QDateTime(),
                        unit.contains("gmt_registration_start")
                            ? QDateTime::fromString(
                                  unit.value("gmt_registration_start")
                                      .toString(),
                                  "yyyy-MM-dd hh:mm:ss")
                            : QDateTime(),
                        unit.contains("gmt_registration_end")
                            ? QDateTime::fromString(
                                  unit.value("gmt_registration_end").toString(),
                                  "yyyy-MM-dd hh:mm:ss")
                            : QDateTime(),
                        EventType{
                            readFile(
                                "localdata/type.json", "id",
                                std::pair<QString, QString>(
                                    "id", unit.value("type_id").toString()))
                                .toInt(),
                            readFile(
                                "localdata/type.json", "type_name",
                                std::pair<QString, QString>(
                                    "id", unit.value("type_id").toString())),
                            readFile(
                                "localdata/type.json", "allow_conflict",
                                std::pair<QString, QString>(
                                    "id", unit.value("type_id").toString()))
                                    .compare("false")
                                ? true
                        ...
                        unit.contains("tag") ? unit.value("tag").toString()
                                             : QString(),
                        static_cast<EventState>(
                            unit.value("state").toString().toInt()),
                        departments});
                }
            }
        }
    ...
    std::vector<Participation> readParticipation(
        const std::pair<QString, QString>& key) {
            ...
            for (int i = 0; i < jsonArr.count(); i++) {
                ...
                if ((!key.first.compare("null") &&
                    ...
                    res.push_back(Participation{
                        ...
                        ParticipationStatus{unit.value("is_registration")
                                                    .toString()
                                                    .compare("false")
                                                ? true
                                                : false,
                                            unit.value("is_participate")
                                                    .toString()
                                                    .compare("false")
                                                ? true
                                                : false,
                                            unit.value("is_subscribe")
                                                    .toString()
                                                    .compare("false")
                                                ? true
                                                : false},
        ...
        if (jsonError.error == QJsonParseError::NoError && jsonDoc.isArray()) {
            ...
            for (int i = 0; i < jsonArr.count(); i++) {
                if (!jsonArr.at(i)
                         ...
                         .value("event_id")
                         .toString()
                         .compare(QString::number(eventId)) &&
                    !jsonArr.at(i)
                         .toObject()
                         .value("user_id")
                         .toString()
    ...
    bool eventDepartmentMatch(int eventId, int departmentId) {
        QByteArray eventDepartment_str =
            readString("localdata/event_department.json");
        ...
        QJsonDocument jsonDoc =
            QJsonDocument::fromJson(eventDepartment_str, &jsonError);
        if (jsonError.error == QJsonParseError::NoError && jsonDoc.isArray()) {
            QJsonArray jsonArr = jsonDoc.array();
            for (int i = 0; i < jsonArr.count(); i++) {
                if (!jsonArr.at(i)
                    ...
                    !jsonArr.at(i)
                         ...
                         .value("department_id")
                         .toString()
                         .compare(QString::number(departmentId))) {
                    return true;
                }
            }
        }
                ...
                if (!jsonArr.at(i)
                    ...
                    res.push_back(DTO_Permission{
                        ...
                        jsonArr.at(i)
                            ...
                            .toString()
                            .toInt(),
                        QDateTime::fromString(jsonArr.at(i)
                                                  .toObject()
                                                  .value("user_id")
                                                  .toString(),
                                              "yyyy-MM-dd hh:mm:ss")});
    ...
    Department readDepartment(const QString& id) {
        ...
        if (jsonError.error == QJsonParseError::NoError && jsonDoc.isArray()) {
            jsonArr = jsonDoc.array();
            for (int i = 0; i < jsonArr.count(); i++) {
                if (!jsonArr.at(i).toObject().value("id").toString().compare(
                        id)) {
                    return Department{
